...
export function CollaborationCta() {
  return (
    <section className="relative isolate overflow-hidden border-y border-[var(--color-border)] bg-[var(--color-card)]">
      <AiMeshBackground />
        ...
        <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          <div>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              ...
              <Link
                ...
                className="inline-flex items-center justify-center gap-2 rounded-md border border-[var(--color-border)] bg-[var(--color-background)]/70 px-5 py-3 text-sm font-medium text-[var(--color-foreground)] transition-colors hover:border-[var(--color-accent)]/40"
              >
                Join Journal Club
              </Link>
            </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {collaborationCards.map(({ icon: Icon, title, text }) => (
              <div key={title} className="min-h-48 rounded-lg border border-[var(--color-border)] bg-[var(--color-background)]/75 p-5 backdrop-blur-sm">
